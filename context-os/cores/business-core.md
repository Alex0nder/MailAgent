...
| v0.4 | **MCP как primary agent surface** (23 tools) | Только REST | Cursor/Codex/remote MCP; discovery `GET /v1/agent` |
...
3. **23 MCP tools**, Agent Skills, npm SDK — агент проходит signup verification **без human OTP check**; QA использует `@mailagent/qa` и `POST /v1/inboxes/open` в CI.
...
| 7 | **Agent без MCP** | Agent не знает как ждать почту | 23 MCP tools + Skill | `@mailagent/mcp`, remote `/mcp` |
...
| Agent integration | REST SDK | **23 MCP tools**, Skills, remote MCP |
...
| Codex | `codex mcp add mailagent -- npx -y -p @mailagent/mcp@0.2.5 mailagent-mcp` |
