...
Manifest: `src/mcp/manifest.ts` — **24 tools**, version `0.8.2`. Handlers: `src/mcp/handlers.ts`, session state: `src/mcp/session.ts`.
...
24 tools in `src/mcp/manifest.ts`. After changes run `npm run sync:context-os` and update AGENTS.md markers.
