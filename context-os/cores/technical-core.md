...
Manifest: `src/mcp/manifest.ts` — **23 tools**, version `0.8.1`. Handlers: `src/mcp/handlers.ts`, session state: `src/mcp/session.ts`.
...
23 tools in `src/mcp/manifest.ts`. After changes run `npm run sync:context-os` and update AGENTS.md markers.
