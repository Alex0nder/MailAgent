#!/usr/bin/env node
/** Stage 0: router-only evaluation (no LLM) */
...
function parseArgs(argv) {
  const opts = { questions: path.join(__dirname, "questions.json"), out: null, pilot: false };
...
function main() {
  ...
  for (const q of questions) {
    const routed = routeQuestion(q.question);
