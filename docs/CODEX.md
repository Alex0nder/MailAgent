...
1. If the next step is unclear, call `mailagent_plan_next` and follow its `nextTool` / `nextPayload`.
2. If sender/service is unclear, call `mailagent_suggest_preset` with sample `from` / `subject`.
3. `mailagent_create_inbox` or `mailagent_verify_signup` with `service: "github"` / `"auth0"`; use returned `expectFrom` for custom senders.
4. Agent fills email on signup form.
5. `mailagent_wait_and_extract` / `mailagent_wait_for_message` with `subjectContains`.
6. OTP or `primaryLink` → next scenario step.
7. `mailagent_delete_inbox` on cleanup.
